...
def get_answer(step, think=None):
    ...
    if think is not None:
        final_answer = (
                    f"<think><Progress Estimation>{think['progress_estimation']}</Progress Estimation>\n"
                    f"<Decision Reasoning>{think['decision_reasoning']}</Decision Reasoning>\n"
                    f"<action>{str(answer)}</action>\n"
                    f"<History Summary>{think['history_summary']}</History Summary>\n</think>"
                )
    else:
        final_answer = answer
    return final_answer
...
class AitwDataset(torch.utils.data.Dataset):
    ...
    def get_sample(self, idx):
        ...
        task = item['task']
        memory = item['memory'] if 'memory' in item.keys() else None
        gt_think = item['gt_think']
        with_think = item['with_think']

        if with_think:
            answer_dict = get_answer(item, think=gt_think)
        else:
        ...
        source = aitw_to_qwen(task, action_history, think=with_think, memory=memory)
